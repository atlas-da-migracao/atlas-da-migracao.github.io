...
- **O controle de revelação (R1–R9) tem melhorias identificadas e ainda não implementadas**, registradas como trabalho futuro, não como bloqueio à publicação atual: (a) documentar explicitamente, num só lugar, os limiares numéricos já em vigor por edição (hoje espalhados entre `pipeline/disclosure_check.py` e esta seção); (b) um limiar próprio, potencialmente mais alto, para a matriz origem-destino, hoje tratada com o mesmo piso das demais células; (c) uma checagem de consistência hierárquica entre níveis geográficos (município → região → UF), para impedir que a diferença entre um total publicado e a soma das células publicadas revele uma célula suprimida; (d) uma regra de supressão por concentração de categoria (uma categoria com perto de 100% ou 0% de um grupo pequeno pode ser tão identificável quanto uma contagem baixa, mesmo acima do piso R1). Nenhuma delas altera um número já publicado -- são camadas adicionais de proteção, calibradas para tornar a supressão mais conservadora, nunca menos.
