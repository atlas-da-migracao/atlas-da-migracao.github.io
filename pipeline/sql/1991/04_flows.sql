-- F2 (edição Censo 1991): Matriz de fluxos migratórios origem->destino (data fixa 01/09/1986 ->
-- 01/09/1991) com caracterização por status, escolaridade (25+), renda domiciliar per capita e
-- idade/sexo, e erro-padrão por fluxo (mesmo estimador de conglomerados de 03_indicators.sql).
-- Universo: migração interna com origem conhecida e origem <> destino (origem_valida).
--
-- Override de pipeline/sql/04_flows.sql -- idêntico a pipeline/sql/2000/04_flows.sql (mesma
...
-- indistinguíveis nesta edição -- o Censo 1991 também não coleta o município de nascimento
-- (MIUFPAIS dá só UF ou país, nunca o município) -- ver pipeline/sql/1991/02_classify.sql e
-- pipeline/sql/1991/MAPEAMENTO_02_classify.md §3. Só os paths mudam, para data/interim/1991/.
PRAGMA threads = 10;
PRAGMA memory_limit = '16GB';

CREATE OR REPLACE TEMP VIEW mig AS
    SELECT * FROM read_parquet('data/interim/1991/pessoas_classificado.parquet') WHERE origem_valida;
...
) TO 'data/interim/1991/fluxos_bruto.parquet' (FORMAT PARQUET);
...
) TO 'data/interim/1991/fluxos_rgi_bruto.parquet' (FORMAT PARQUET);
...
COPY (
    ...
    SELECT b.*, SQRT(GREATEST(COALESCE(v.v, 0), 0)) AS se,
           CASE WHEN b.total > 0 THEN 100.0 * SQRT(GREATEST(COALESCE(v.v, 0), 0)) / b.total END AS cv
    FROM base b LEFT JOIN var v ON v.origem = b.origem AND v.destino = b.destino
) TO 'data/interim/1991/fluxos_rgint_bruto.parquet' (FORMAT PARQUET);
...
COPY (
    ...
    SELECT b.*, SQRT(GREATEST(COALESCE(v.v, 0), 0)) AS se,
           CASE WHEN b.total > 0 THEN 100.0 * SQRT(GREATEST(COALESCE(v.v, 0), 0)) / b.total END AS cv
    FROM base b LEFT JOIN var v ON v.origem = b.origem AND v.destino = b.destino
) TO 'data/interim/1991/fluxos_uf_bruto.parquet' (FORMAT PARQUET);
