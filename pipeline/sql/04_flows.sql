-- F2: Matriz de fluxos migratórios origem->destino (data fixa 2017->2022) com
-- caracterização por status, escolaridade (25+), renda domiciliar per capita e idade/sexo,
...
CREATE OR REPLACE TEMP VIEW mig AS
    SELECT * FROM read_parquet('data/interim/pessoas_classificado.parquet') WHERE origem_valida;
...
COPY (
    WITH base AS (
        SELECT
            ...
            COALESCE(SUM(peso) FILTER (WHERE status = 'primeira_saida'), 0)    AS st_primeira_saida,
            COALESCE(SUM(peso) FILTER (WHERE status = 'etapas_multiplas'), 0)  AS st_etapas_multiplas,
...
) TO 'data/interim/fluxos_bruto.parquet' (FORMAT PARQUET);
...
) TO 'data/interim/fluxos_rgi_bruto.parquet' (FORMAT PARQUET);
...
COPY (
    ...
    SELECT b.*, SQRT(GREATEST(COALESCE(v.v, 0), 0)) AS se,
           CASE WHEN b.total > 0 THEN 100.0 * SQRT(GREATEST(COALESCE(v.v, 0), 0)) / b.total END AS cv
    FROM base b LEFT JOIN var v ON v.origem = b.origem AND v.destino = b.destino
) TO 'data/interim/fluxos_rgint_bruto.parquet' (FORMAT PARQUET);
...
COPY (
    ...
    SELECT b.*, SQRT(GREATEST(COALESCE(v.v, 0), 0)) AS se,
           CASE WHEN b.total > 0 THEN 100.0 * SQRT(GREATEST(COALESCE(v.v, 0), 0)) / b.total END AS cv
    FROM base b LEFT JOIN var v ON v.origem = b.origem AND v.destino = b.destino
) TO 'data/interim/fluxos_uf_bruto.parquet' (FORMAT PARQUET);
