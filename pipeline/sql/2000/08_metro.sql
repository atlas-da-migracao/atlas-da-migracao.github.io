-- F5b (edição Censo 2000): Recortes metropolitanos, migração intrametropolitana e o cruzamento
-- entre migração intra-RM e deslocamento pendular (a pergunta da desconcentração metropolitana:
-- quem se mudou do núcleo para a periferia continua trabalhando no núcleo?).
-- Override de pipeline/sql/08_metro.sql -- idêntico na estrutura a pipeline/sql/2010/08_metro.sql,
-- exceto que 2000 é AINDA mais pobre que 2010 no bloco de deslocamento (ver
-- MAPEAMENTO_02_classify.md §11 e docs/METODOLOGIA.md, "Edição Censo 2000 e comparabilidade"):
--   1. `pct_coletivo`: sempre NULL -- o Censo 2000 não pergunta meio de transporte (modo_grupo
--      é sempre NULL, mesma razão de pipeline/sql/2000/07_pendular.sql).
--   2. `tempo_mediano`: sempre NULL -- tempo_desloc_min é sempre NULL em 2000; o Censo 2000 não
--      registra tempo de deslocamento nem em minutos nem em faixas (nem sequer o que 2010 tinha
--      em V0662). Explicitado com CAST(NULL AS DOUBLE) em vez de deixar o
--      MEDIAN(tempo_desloc_min) convergir sozinho para NULL, para documentar a ausência do dado
--      em vez de depender de um efeito colateral do agregador.
--   3. `pct_diario`: sempre NULL -- DIVERGÊNCIA FRENTE A 2010, que tinha `pct_diario` via
--      `retorna_3dias`/V0661 ("retorna diariamente"). O Censo 2000 não tem quesito NENHUM de
--      frequência de retorno: o bloco de deslocamento é só o quesito 4.27 (município de
--      trabalho/estudo) e mais nada.
-- Núcleo metropolitano: pipeline/rm_nucleo.csv -- mesmo arquivo, compartilhado com as edições
-- 2022/2010 (ver pipeline/build_rm_nucleo.py). O recorte de RM é o de 2022 aplicado
-- retroativamente por código de município (pipeline/build_ref.py::_build_outra_edicao); os
-- municípios criados depois de 2000 não existem nesta edição, então as RMs que os contêm ficam
-- com um município a menos (ver aviso de build_ref.py na execução).
PRAGMA threads = 10;
...
LEFT JOIN read_parquet('data/interim/2000/municipios_bruto.parquet') m USING (cd_mun)
WHERE r.cd_rm IS NOT NULL;

COPY (SELECT * FROM rm) TO 'data/interim/2000/rm_bruto.parquet' (FORMAT PARQUET);
...
) TO 'data/interim/2000/rm_fluxos_intra_bruto.parquet' (FORMAT PARQUET);

-- ---------- CRUZAMENTO: migrantes intra-RM que fazem deslocamento pendular ----------
-- Tripla "morava em (1995) -> mora em (2000) -> trabalha em (2000)".
...
COPY (
    SELECT cd_rm, df_mun AS origem_mig, cd_mun AS destino_mig, destino_trab, classe_trab,
           SUM(peso) AS total, COUNT(*) AS n, COUNT(DISTINCT controle) AS ndom,
           CAST(NULL AS DOUBLE) AS pct_diario,   -- inexistente em 2000 (sem quesito de frequência de retorno)
           CAST(NULL AS DOUBLE) AS pct_coletivo  -- inexistente em 2000 (sem meio de transporte)
    FROM mig_rm_ocup GROUP BY 1, 2, 3, 4, 5
) TO 'data/interim/2000/rm_mig_pendular_bruto.parquet' (FORMAT PARQUET);
...
) TO 'data/interim/2000/rm_mig_pendular_resumo_bruto.parquet' (FORMAT PARQUET);
...
) TO 'data/interim/2000/rm_mig_estudo_bruto.parquet' (FORMAT PARQUET);

-- ---------- resumo por região metropolitana ----------
COPY (
    WITH pop_rm AS (
        SELECT r.cd_rm, SUM(m.pop) AS pop, COUNT(*) AS n_municipios
        FROM rm r JOIN read_parquet('data/interim/2000/municipios_bruto.parquet') m USING (cd_mun)
        GROUP BY 1
    ), intra AS (
        ...
        SELECT r.cd_rm,
               ...
               CAST(NULL AS DOUBLE)                                     AS pct_diario      -- inexistente em 2000 (divergência frente a 2010)
...
) TO 'data/interim/2000/rm_resumo_bruto.parquet' (FORMAT PARQUET);
