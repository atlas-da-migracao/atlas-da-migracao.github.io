-- F5b (edição Censo 2010): Recortes metropolitanos, migração intrametropolitana e o cruzamento
...
-- Override de pipeline/sql/08_metro.sql -- idêntico, exceto nos pontos onde o questionário de
-- 2010 diverge do de 2022 (ver docs/METODOLOGIA.md, seção "Edição Censo 2010 e comparabilidade"):
--   1. `pct_coletivo`: sempre NULL -- o Censo 2010 não pergunta meio de transporte (modo_grupo
--      é sempre NULL, mesma razão de pipeline/sql/2010/07_pendular.sql).
--   2. `tempo_mediano`: sempre NULL -- tempo_desloc_min é sempre NULL em 2010; o Censo 2010 só
--      registra o tempo de deslocamento em 5 faixas categóricas de V0662, não em minutos (ver
--      pipeline/sql/2010/07_pendular.sql, dimensão "tempo"). Explicitado com CAST(NULL AS
--      DOUBLE) em vez de deixar o MEDIAN(tempo_desloc_min) convergir sozinho para NULL, para
--      documentar a ausência do dado em vez de depender de um efeito colateral do agregador.
--   3. `pct_diario`: calculado a partir de `retorna_3dias`, que em 2010 vem de V0661 ("retorna
--      diariamente"; códigos iguais aos de 2022, mas a pergunta muda -- 2022 pergunta quantos
--      dias por semana volta para casa, 2010 só se volta todo dia).
-- Núcleo metropolitano: pipeline/rm_nucleo.csv -- mesmo arquivo, compartilhado com a edição
-- 2022 (ver pipeline/build_rm_nucleo.py). O recorte de RM é o de 2022 aplicado retroativamente
-- por código de município (pipeline/build_ref.py::_build_outra_edicao); os 5 municípios criados
-- em 2013 não existem em 2010, então as RMs que os contêm ficam com um município a menos.
PRAGMA threads = 10;
...
CREATE OR REPLACE TEMP VIEW ref AS
    SELECT * FROM read_parquet('data/interim/2010/municipios_ref.parquet');
...
LEFT JOIN read_parquet('data/interim/2010/municipios_bruto.parquet') m USING (cd_mun)
WHERE r.cd_rm IS NOT NULL;

COPY (SELECT * FROM rm) TO 'data/interim/2010/rm_bruto.parquet' (FORMAT PARQUET);
...
) TO 'data/interim/2010/rm_fluxos_intra_bruto.parquet' (FORMAT PARQUET);

-- ---------- CRUZAMENTO: migrantes intra-RM que fazem deslocamento pendular ----------
-- Tripla "morava em (2005) -> mora em (2010) -> trabalha em (2010)".
...
) TO 'data/interim/2010/rm_mig_pendular_bruto.parquet' (FORMAT PARQUET);
...
COPY (
    WITH m AS (
        ...
        SELECT cd_rm, cd_mun,
               ...
               CAST(NULL AS DOUBLE)                                      AS tempo_mediano  -- inexistente em 2010
...
) TO 'data/interim/2010/rm_mig_pendular_resumo_bruto.parquet' (FORMAT PARQUET);
...
) TO 'data/interim/2010/rm_mig_estudo_bruto.parquet' (FORMAT PARQUET);

-- ---------- resumo por região metropolitana ----------
COPY (
    WITH pop_rm AS (
        SELECT r.cd_rm, SUM(m.pop) AS pop, COUNT(*) AS n_municipios
        FROM rm r JOIN read_parquet('data/interim/2010/municipios_bruto.parquet') m USING (cd_mun)
        GROUP BY 1
    ), intra AS (
...
) TO 'data/interim/2010/rm_resumo_bruto.parquet' (FORMAT PARQUET);
