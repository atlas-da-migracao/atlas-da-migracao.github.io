-- F2 (edição Censo 2010): Matriz de fluxos migratórios origem->destino (data fixa 2005->2010)
...
--
-- Override de pipeline/sql/04_flows.sql -- idêntico, exceto na caracterização por status:
-- 2010 publica 3 colunas (st_retorno_natal, st_nao_natural, st_nascido_exterior), não 4,
-- porque `primeira_saida`/`etapas_multiplas` são indistinguíveis nesta edição (o Censo 2010
-- não coleta o município de nascimento -- ver pipeline/sql/2010/02_classify.sql e
-- docs/METODOLOGIA.md).
PRAGMA threads = 10;
PRAGMA memory_limit = '16GB';

CREATE OR REPLACE TEMP VIEW mig AS
    SELECT * FROM read_parquet('data/interim/2010/pessoas_classificado.parquet') WHERE origem_valida;
...
) TO 'data/interim/2010/fluxos_bruto.parquet' (FORMAT PARQUET);
...
) TO 'data/interim/2010/fluxos_rgi_bruto.parquet' (FORMAT PARQUET);
...
COPY (
    ...
    SELECT b.*, SQRT(GREATEST(COALESCE(v.v, 0), 0)) AS se,
           CASE WHEN b.total > 0 THEN 100.0 * SQRT(GREATEST(COALESCE(v.v, 0), 0)) / b.total END AS cv
    FROM base b LEFT JOIN var v ON v.origem = b.origem AND v.destino = b.destino
) TO 'data/interim/2010/fluxos_rgint_bruto.parquet' (FORMAT PARQUET);
...
COPY (
    ...
    SELECT b.*, SQRT(GREATEST(COALESCE(v.v, 0), 0)) AS se,
           CASE WHEN b.total > 0 THEN 100.0 * SQRT(GREATEST(COALESCE(v.v, 0), 0)) / b.total END AS cv
    FROM base b LEFT JOIN var v ON v.origem = b.origem AND v.destino = b.destino
) TO 'data/interim/2010/fluxos_uf_bruto.parquet' (FORMAT PARQUET);
