...
Plano da seção de análise comparativa entre censos ("Ao longo dos censos", branch `comparacao-censos`): `~/.claude/plans/vamos-retomar-o-plano-expressive-lobster.md`. Ela lê as cinco edições juntas a partir de `data/processed` (nunca microdados, nunca o pipeline), **sem AMC** — base territorial é o recorte de 2022 comparado retroativamente, com a genealogia municipal como metadado e o limiar de cobertura de 0,90 como contrapartida. Ver `docs/METODOLOGIA.md`, seção "Comparação entre censos (F12)", `docs/genealogia.md` e o aviso 1 de `docs/EDICOES.md`.
...
python pipeline/build_genealogia.py  # genealogia municipal (município de 2022 -> território de origem em cada censo) -> pipeline/genealogia_municipios.csv + area_km2.parquet; metadado de comparabilidade, NÃO é AMC
python pipeline/build_series.py      # série "Ao longo dos censos": lê as cinco pastas de data/processed (nunca microdados) -> data/processed/series/ com .gate_ok próprio
